...
actor AgentPassport {
  public type ChainRef = {
    namespace : Text;
    reference : Text;
  };

  public type CommitmentSet = {
    identity : Blob;
    doctrine : Blob;
    capability : Blob;
    runtime : Blob;
  ...
  func validUri(uri : Text) : Bool {
    Text.startsWith(uri, #text("ipfs://")) or
    Text.startsWith(uri, #text("ar://")) or
    Text.startsWith(uri, #text("https://"));
  };
    ...
    for ((_, passport) in passports.entries()) {
      if (passport.agentPrincipal == agent) {
        switch (latest) {
          ...
          case (?current) {
            if (passport.version > current.version) { latest := ?passport };
          };
        };
      };
    };
  ...
  public shared ({ caller }) func anchorEvidence(
    ...
    if (passport.owner != caller and passport.agentPrincipal != caller) {
      return #err("caller is not authorized for this passport");
    };
    if (Text.size(transactionHash) == 0 or Text.size(eventType) == 0) {
      return #err("event type and transaction hash are required");
    };
    switch (metadataUri) {
  ...
  public query func getPassport(id : Nat) : async ?Passport {
    passports.get(id);
  };

  public query func getActivePassport(agent : Principal) : async ?Passport {
    switch (latestFor(agent)) {
      case (?passport) { if (passport.active) ?passport else null };
      case null { null };
    };
  };
  ...
  public query func listPassportHistory(agent : Principal) : async [Passport] {
    ...
    for ((_, passport) in passports.entries()) {
      if (passport.agentPrincipal == agent) out.add(passport);
    };
  ...
  public query func listEvidence(passportId : Nat) : async [EvidenceAnchor] {
    ...
    for ((_, anchor) in evidence.entries()) {
      if (anchor.passportId == passportId) out.add(anchor);
    };
    Buffer.toArray(out);
  };
  ...
  public query func verifyCommitments(id : Nat, expected : CommitmentSet) : async Bool {
    switch (passports.get(id)) {
      case null { false };
      case (?passport) {
        passport.commitments.identity == expected.identity and
        passport.commitments.doctrine == expected.doctrine and
        passport.commitments.capability == expected.capability and
        passport.commitments.runtime == expected.runtime;
      };
    };
  };
